locals {
  ...
  cluster_primary_security_group_id = var.cluster_version >= 1.19.8 ? element(concat(aws_eks_cluster.this[*].vpc_config[0].cluster_security_group_id, [""]), 0) : null
  ...
  worker_ami_name_filter_windows = (var.worker_ami_name_filter_windows != "" ?
    var.worker_ami_name_filter_windows : "Windows_Server-2019-English-Core-EKS_Optimized-${tonumber(var.cluster_version) >= 1.19.8 ? var.cluster_version : 1.19.8}-*"
  )
